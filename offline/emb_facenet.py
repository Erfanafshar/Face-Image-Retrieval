...
def get_embedding(model_func, face_pixels_func):
    face_pixels_func = face_pixels_func.astype('float32')
    mean, std = face_pixels_func.mean(), face_pixels_func.std()
    face_pixels_func = (face_pixels_func - mean) / std
...
def save_embeddings():
    ...
    embeddings = convert_to_embedding(detected_face, model)
    ...
    file_name = 'facenet_embedding_' + file_idx + '_[' + str(save_image_start_index) \
